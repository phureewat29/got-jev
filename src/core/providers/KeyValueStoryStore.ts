...
/**
 * A `StoryStore` over whatever `KeyValueStore` is in context: one Schema-encoded document
 * per session. The Redis store and the memory store differ only in the backend they hand it.
 */
export const make = Effect.gen(function* () {
  const documents = (yield* KeyValueStore.KeyValueStore).forSchema(StoryState);

  /**
   * A document this code can no longer decode reads as no story at all. Stories are
   * ephemeral here: they carry a TTL and the reader has a button to start again, so a
   * fresh opening beats a 500 about a document they never knew existed.
   *
   * Only a decode failure is forgiven. A backend that cannot be reached still fails,
   * because pretending a reachable store is empty would overwrite a story that is there.
   */
